"""
Layer 2 v4 — Mucus-Clean LightGBM (LOPO CV)
=============================================
All mucus-derived features are excluded at training time and at inference.
User-reported cervical mucus is still accepted as input so the fusion layer
can apply its own mucus bonus (inference-time signal), but it is not fed
to the model itself.

Output dict shape is identical to model_v1 and model_v3.
"""
...
from ..config import (
    ARTIFACTS_DIR,
    LAYER2_V4_FEATURE_COLUMNS_FILE,
    LAYER2_V4_PIPELINE_FILE,
...
def _build_v4_frame(
    ...
    # Feature columns saved at training time already exclude all mucus columns.
    feature_cols = load_artifact("v4_feature_cols", ARTIFACTS_DIR / LAYER2_V4_FEATURE_COLUMNS_FILE)
    # Pass mucus="unknown" so the feature builder doesn't produce mucus columns
    # that would end up out-of-distribution for this model.
    full_row = build_v3_feature_row(
...
def get_layer2_v4_output(
    ...
    *,
    ...
    """Run v4 inference and return a standardized output dict."""
    pipeline    = load_artifact("v4_pipeline", ARTIFACTS_DIR / LAYER2_V4_PIPELINE_FILE)
    ...
    return {
        ...
        "explanations": [
            "v4 mucus-clean model used symptom load and cycle position to refine the phase estimate."
        ],
